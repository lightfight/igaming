...
public class AppSyslogAppender extends SyslogAppenderBase<ILoggingEvent> {
...
//		return new SyslogOutputStream(getSyslogHost(), getPort());
		return new SyslogOutputStream(getSyslogHost(), 554);
	}
	...
	/**
	 * Stack trace lines are sent to the syslog server separately from the main message For stack trace lines, the stackTracePattern is used instead of {@link #suffixPattern}. The <b>stackTracePattern</b> option allows specification of a separately format for the non-standardized part of stack trace lines.
	 *
